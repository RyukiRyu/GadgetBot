...
const getLatest = async () => {
    const html = await getDataFromUrl('/reviews.php3');

    const $ = cheerio.load(html);
    const json = [];
    let data = $('.module-latest');
    data.each((index, el) => {
        const title = $(el).find('h4').text();
        if(title === 'Latest devices') {
            const items = $(el).find('.module-phones-link');
            items.each((i, el) => {
                const link = $(el).attr('href');
                const img = $(el).find('img').attr('src');
                const name = $(el).text();
                json.push({ link, img, name });
            });
        }
    });
